...
;(function($) {
	...
	$.fn.twitterSearch.formatter = function(json, opts) {
		...
		if (opts.time && pretty)
			str += ' <span class="twitterSearchTime"><br/>('+ pretty +')<br/><br/></span>'
		 str += '<div class="twitterSearchText" style="margin-left:-40px;">' + text + '</div><br/><br/></div></div>';
		 return str;
	};
	...
	$.fn.twitterSearch.defaults = {
		...
		birdSrc: 'modules/blocktwittercycle/img/tweet.png', // twitter bird image
		...
		css: {
			...
			bird:  { width: '50px', height: '20px', position: 'absolute', left: '140px', top: '-20px', border: 'none' },
			...
			img:   { 'float': 'right', width: '48px', height: '48px' },
